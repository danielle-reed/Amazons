...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\s+black", this:: doAuto),
        new Command("auto\\s+white", this:: doAuto),
        new Command("manual\\s+black", this:: doManual),
        new Command("manual\\s+white", this:: doManual),
    ...
    public void executeCommand(String cmnd) {
        if (_logFile != null) {
    ...
    /** Make the color inputed a auto/ai player with param MAT. */
    private void doAuto(Matcher mat) {
        if (mat.group(0).equals("auto black")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else {
    ...
    /** Make the color inputed a manual player with param MAT. */
    private void doManual(Matcher mat) {
        if (mat.group(0).equals("manual black")) {
    ...
    /** Make a move with para MAT. */
    private void doMove(Matcher mat) {
        Move temp = Move.mv(mat.group(0));
        board().makeMove(temp);
